...
var JsonView = (function (exports) {
  ...
  function createNodeElement(node) {
    ...
    if (/^[0-1]+$/.test(node.value) && node.value.length >= 4)
    {
        var pre = String(node.value);
        var key = pad(node.key,2);
        var converted = ""
        for(let x = 0; x < pre.length; x++)
        {
            ...
            if(pre.charAt(x) == '0')
            {
                converted += "<span style='color:blue'>0</span>";
            }
            else
            {
                converted += "<span style='color:red'>1</span>";
            }
